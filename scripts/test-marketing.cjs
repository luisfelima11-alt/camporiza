...
assert.equal(CONTENT.find(p => p.id === '2026-10-06-area-pequena').assets.length, 3)
assert.equal(CONTENT.find(p => p.id === '2026-10-10-sinal-lavoura').assets.length, 1)
assert.equal(CONTENT.filter(p => p.id.startsWith('2026-10-0') || p.id === '2026-10-10-sinal-lavoura').every(p => p.caption.includes('Imagem ilustrativa criada com IA.')), true)
